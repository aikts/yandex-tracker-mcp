...
- **`project_update_checklist`**: Массово изменить существующие пункты чек-листа. Требуется `entity_id` и `items` (объекты `{id, text, checked?, assignee?, deadline?}`). Передавать нужно все текущие пункты: неполный список API отклоняет с ошибкой 500, а количество пунктов здесь менять нельзя (проверено на живом API). Для изменения состава используйте инструменты добавления/удаления пункта
