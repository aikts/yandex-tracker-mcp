...
- **`users_get_all`**: Получить информацию об учетных записях пользователей, зарегистрированных в организации
  ...
  - Пагинация: вызывайте повторно с увеличивающимся `page`, пока не будет возвращён пустой список
...
- **`issue_templates_get_all`**: Получить все шаблоны задач, настроенные в Яндекс Трекере
  ...
  - Возвращает шаблоны, которые команды используют для багов, инцидентов и другой типовой работы
...
- **`comment_templates_get_all`**: Получить все шаблоны комментариев, настроенные в Яндекс Трекере
  - Параметры:
    ...
    - `page` (целое, опционально): Номер страницы, по умолчанию возвращаются все страницы
    - `per_page` (целое, опционально): Количество элементов на странице (по умолчанию: 50)
  - Возвращает формулировки, которые команды переиспользуют при ответах в задачах
