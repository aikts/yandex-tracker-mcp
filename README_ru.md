...
- **`queue_get_fields`**: Получить поля для конкретной очереди
  - Параметры: `queue_id` (строка, ключ очереди, например "SOMEPROJECT")
  - Возвращает список полей, которые можно использовать при создании задач в этой очереди
...
- **`issue_create`**: Создать новую задачу в очереди
  - Параметры:
    ...
    - `fields` (объект, опционально): Дополнительные поля для установки при создании задачи. **ВАЖНО**: Перед созданием задачи вы ДОЛЖНЫ вызвать `queue_get_fields` для получения доступных полей очереди и `queue_get_local_fields` для получения пользовательских полей очереди. Поля с `schema.required=true` являются обязательными. Используйте свойство `id` поля в качестве ключа в этом объекте (например, `{"fieldId": "value"}`)
