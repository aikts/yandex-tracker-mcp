...
| `worklogs_search` | Записи о затраченном времени по всем задачам организации, с фильтрами по автору и дате создания | `created_by`, `created_at_from`, `created_at_to` (ISO 8601, `2024-01-01T00:00:00.000+0000`) |
...
- `worklogs_search` ищет по всей организации, а не по очереди: `TRACKER_LIMIT_QUEUES` его не фильтрует, и задачи, которые называют найденные записи, могут быть вне списка разрешённых. Без единого фильтра возвращаются все записи, доступные токену — задачу целиком читает `issue_get_worklogs`.
