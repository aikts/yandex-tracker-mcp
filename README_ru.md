...
- **`worklogs_search`**: Поиск записей трудозатрат по всем задачам
  - Параметры (все опциональные):
    - `created_by` (строка): Логин или ID пользователя, создавшего запись трудозатрат (например, 'john.doe' или '1234567890')
    - `created_at_from` (строка): Начало временного диапазона в формате ISO 8601 (например, '2024-01-01T00:00:00.000+0000')
    - `created_at_to` (строка): Конец временного диапазона в формате ISO 8601 (например, '2024-12-31T23:59:59.999+0000')
  - Возвращает список записей трудозатрат, соответствующих указанным фильтрам
