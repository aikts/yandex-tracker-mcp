...
- **`get_queue_resolutions`**: Получить резолюции, доступные в конкретной очереди
  - Параметры: `queue_id` (строка, ключ очереди, например "SOMEPROJECT")
  - Возвращает список резолюций, которые можно использовать при закрытии задач в этой очереди
  - Используйте это для поиска допустимых ID резолюций для инструмента `issue_close`
  - Учитывает ограничения `TRACKER_LIMIT_QUEUES`
...
- **`issue_close`**: Закрыть задачу с резолюцией (удобный инструмент)
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123"): Ключ задачи
    - `resolution_id` (строка, обязательно): ID резолюции для установки при закрытии. **ВАЖНО**: Должен быть одним из ID, возвращенных инструментом `get_resolutions` (например, 'fixed', 'wontFix', 'duplicate')
  ...
  - **Примечание по использованию**: Вы ДОЛЖНЫ сначала вызвать `get_resolutions` для получения доступных резолюций, затем передать один из возвращенных ID резолюций. Этот инструмент упрощает процесс закрытия, автоматически находя и выполняя соответствующий переход.
