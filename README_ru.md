...
- **`issue_get_changelog`**: Получить историю изменений (changelog) задачи
  ...
  - Возвращает объект с `entries` (переходы статусов и правки полей, включая кто, что изменил `from` → `to` и когда) и `next_cursor` (передайте обратно как `cursor` для следующей страницы; `null`, когда страниц больше нет)
