...
- **`issue_add_checklist_items`**: Добавить пункты в чек-лист задачи
  - Параметры:
    - `issue_id` (строка, обязательный, формат: "QUEUE-123")
    - `items` (массив, обязательный, минимум один элемент): Пункты чек-листа, каждый содержит:
      - `text` (строка, обязательный): Текст пункта чек-листа
      - `checked` (булево, опционально): Отметка о выполнении пункта
      - `assignee` (строка, опционально): Логин или ID пользователя — исполнителя пункта
      - `deadline` (объект, опционально): `date` (дата и время, при отсутствии таймзоны используется UTC) и `deadline_type` (`date` или `quarter`)
  - Создаёт чек-лист, если его ещё нет в задаче, иначе добавляет пункты в конец в указанном порядке
  - Возвращает чек-лист задачи после добавления пунктов

- **`issue_update_checklist_item`**: Изменить пункт чек-листа задачи
  - Параметры:
    - `issue_id` (строка, обязательный, формат: "QUEUE-123")
    - `item_id` (строка, обязательный): ID пункта чек-листа из результата `issue_get_checklist`
    - `text` (строка, опционально): Новый текст пункта чек-листа
    - `checked` (булево, опционально): Отметить пункт выполненным (`true`) или снять отметку (`false`)
    - `assignee` (строка, опционально): Логин или ID пользователя — исполнителя пункта
    - `deadline` (объект, опционально): `date` (дата и время, при отсутствии таймзоны используется UTC) и `deadline_type` (`date` или `quarter`)
  - Изменяются только переданные параметры; если `text` не передан, используется текущий текст пункта (Трекер требует его в каждом запросе)
  - Возвращает чек-лист задачи после изменения

- **`issue_delete_checklist_item`**: Удалить пункт из чек-листа задачи
  - Параметры:
    - `issue_id` (строка, обязательный, формат: "QUEUE-123")
    - `item_id` (строка, обязательный): ID пункта чек-листа из результата `issue_get_checklist`
  - Возвращает оставшиеся пункты чек-листа задачи
...
- **`TRACKER_READ_ONLY_QUEUES`** — список очередей только для чтения. Инструменты
  ...
  (создание/обновление/перемещение/комментарий/списание времени/связь/чек-лист,
  создание версии очереди) к указанной очереди отклоняется, а чтение продолжает
  работать.
