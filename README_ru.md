...
- `TRACKER_TOKEN` - Ваш OAuth токен Яндекс.Трекера (обязательно)
...
- **`issues_find`**: Поиск задач с использованием [языка запросов Яндекс.Трекера](https://yandex.ru/support/tracker/ru/user/query-filter)
  ...
  - Возвращает до 500 задач на страницу
...
```env
# Обязательно - OAuth токен API Яндекс.Трекера
...
TRACKER_BASE_URL=https://api.tracker.yandex.net  # По умолчанию: https://api.tracker.yandex.net
