...
- **`issue_add_checklist_items`**: Добавить пункты в чек-лист задачи (чек-лист создаётся, если его нет)
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `items` (массив, обязательно): Пункты для добавления, по порядку. Каждый принимает `text` (строка, обязательно), `checked` (булево, опционально), `assignee` (логин или uid, опционально), `deadline` (объект, опционально: `{"date": "2026-08-20T00:00:00", "deadline_type": "date"}`)
  - Возвращает чек-лист задачи после изменения

- **`issue_update_checklist_item`**: Изменить один пункт чек-листа, например отметить выполненным
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `checklist_item_id` (строка, обязательно): ID пункта, как его возвращает `issue_get_checklist`
    - `text` (строка, опционально), `checked` (булево, опционально), `assignee` (логин или uid, опционально), `deadline` (объект, опционально: `deadline_type` — `"date"` или `"quarter"`)
    - `clear_assignee` / `clear_deadline` (булево, опционально): снять текущего исполнителя или дедлайн; нельзя передавать вместе с самим полем
  - Нужно передать хотя бы одно опциональное поле; непереданные поля сохраняют значение
  - Возвращает чек-лист задачи после изменения

- **`issue_delete_checklist_item`**: Удалить один пункт чек-листа
  - Параметры: `issue_id` (строка, обязательно, формат: "QUEUE-123"), `checklist_item_id` (строка, обязательно)
  - Возвращает чек-лист задачи после изменения
