...
- **`project_get`**: Получить проект по id или shortId
  - Параметры: `entity_id` (строка, обязательно); `fields` (массив строк, опционально, по умолчанию базовый набор полей)
...
- **`project_find`**: Поиск проектов по подстроке в названии и/или фильтрам полей
  - Параметры (все опциональны): `input`, `filter`, `order_by`, `order_asc`, `root_only`, `page`, `per_page`, `fields`
  - Возвращает `{values, hits, pages}`

- **`project_get_comments`**: Получить страницу комментариев проекта
  - Параметры: `entity_id` (обязательно); `per_page` (опционально, по умолчанию 50); `cursor` (опционально, значение `next_cursor` из предыдущего вызова); `fields` (опционально)
  - Возвращает `{comments, next_cursor}` — структура та же, что у комментариев задач. Передавайте `next_cursor` как `cursor`, пока он не станет null

- **`project_create`**: Создать проект. Требуется `summary`; принимает `description`, `lead`, `team_users`, `clients`, `followers`, `start`, `end`, `tags`, `entity_status`, `parent_entity`, `team_access` и `links`
- **`project_update`**: Изменить любое из этих полей. Также принимает `comment` и `version` (оптимистичная блокировка). `links` **добавляет** связи; API их не возвращает, поэтому прочитать или удалить их нельзя, а апдейт только со связями отклоняется
- **`project_delete`**: Удалить проект. Принимает `with_board` для удаления связанной доски
- **`project_add_comment`** / **`project_update_comment`** / **`project_delete_comment`**: Работа с комментариями проекта. Для уведомления пользователей используйте `summonees` / `maillist_summonees`, а не `@login` в тексте
- **`project_add_checklist_item`** / **`project_update_checklist_item`** / **`project_move_checklist_item`** / **`project_delete_checklist_item`** / **`project_update_checklist`** / **`project_delete_checklist`**: Работа с чек-листом проекта. Все возвращают сущность целиком — запросите `checklistItems` через `fields`, чтобы увидеть пункты

**`portfolio_*`**: те же инструменты для портфелей (портфель объединяет проекты и/или другие портфели).

**`goal_*`**: те же инструменты для целей, но без чек-листов (API их для целей не поддерживает), без `start` и без `with_board` у `goal_delete`. У целей свой набор значений `entityStatus` (`draft`, `according_to_plan`, `at_risk`, `blocked`, `achieved`, `partially_achieved`, `not_achieved`, `exceeded`, `cancelled`).

Все инструменты создания/изменения принимают тот же селектор `fields`, что и инструменты чтения, и возвращают созданную/изменённую сущность. Инструменты записи регистрируются, только если не задан `TRACKER_READ_ONLY`.

Метрики (`metricItems`) и ключевые результаты цели (`keyResultItems`) доступны только на чтение и не входят в набор полей по умолчанию — запрашивайте их через `fields`. Массовые изменения не поддерживаются.
...
- **`issue_templates_get_all`**: Получить все шаблоны задач, настроенные в Яндекс Трекере
  ...
  - Возвращает `{values, hits, pages}`; у каждого шаблона есть идентификатор, имя, очередь и значения `fieldTemplates`, которые он подставляет в поля задачи

- **`issue_template_get`**: Получить один шаблон задачи по его идентификатору
  - Параметры: `template_id` (строка, из результата `issue_templates_get_all`)
  - Возвращает шаблон; прочитайте его перед `issue_create` и передайте значения в аргументы этого инструмента

- **`comment_templates_get_all`**: Получить все шаблоны комментариев, настроенные в Яндекс Трекере
  - Параметры: те же, что у `issue_templates_get_all`
  - Возвращает `{values, hits, pages}`; у каждого шаблона есть идентификатор, имя, описание, очередь, текст комментария `template` и списки призываемых `summonees` / `maillistSummonees`

- **`comment_template_get`**: Получить один шаблон комментария по его идентификатору
  - Параметры: `template_id` (строка, из результата `comment_templates_get_all`)
  - Возвращает шаблон; прочитайте его перед `issue_add_comment` и передайте значения в аргументы этого инструмента

Шаблоны доступны только на чтение: в API Трекера нет способа создать задачу или комментарий *из* шаблона, поэтому у `issue_create` и `issue_add_comment` нет параметра `template_id`.

Все четыре инструмента учитывают `TRACKER_LIMIT_QUEUES`: шаблоны запрещённых очередей исключаются из списков и отклоняются при прямом обращении, шаблоны без очереди остаются доступными.
...
- **`boards_get_all`**: Получить Agile-доски, доступные в организации
  - Параметры:
    ...
    - `fields` (массив строк, опционально): Поля, которые нужно вернуть (например, `["id", "name"]`)
    - `cursor` (целое число, опционально): Значение `next_cursor` из предыдущего вызова. Для первой страницы оставьте пустым
    - `per_page` (целое число, опционально, по умолчанию 25): Количество досок на странице
  - Возвращает `{boards, next_cursor}`: доски с id, названием, версией, колонками и настройками. Передавайте `next_cursor` обратно в `cursor`, пока он не вернётся `null`
  ...
  - `queue` учитывает ограничения `TRACKER_LIMIT_QUEUES`

- **`board_get`**: Получить одну Agile-доску с её настройками
  - Параметры: `board_id` (целое число, идентификатор доски из `boards_get_all`); `fields` (массив строк, опционально)
  - Возвращает доску с `autoFilterSettings` (фильтр, по которому доска собирает задачи), `estimateBy`, `useRanking` и производственным календарём `calendar`
...
- **`board_get_columns`**: Получить колонки Agile-доски вместе со статусами задач
  ...
  - Возвращает каждую колонку с `id`, `name` и списком статусов `statuses`, которые в неё попадают

- **`board_get_sprints`**: Получить все спринты конкретной Agile-доски
  - Параметры: `board_id` (целое число, идентификатор доски из `boards_get_all`); `fields` (массив строк, опционально)
  - Возвращает спринты с id, названием, статусом (`draft`, `in_progress`, `released`, `archived`), признаком архивности и плановыми/фактическими датами. У доски, которая не является scrum-доской, спринтов нет, и вызов отклоняется
  - Используйте полученный `id` спринта, чтобы поместить задачу в спринт с помощью `issue_create` или `issue_update`

Доски относятся ко всей организации, а не к очереди, поэтому по `TRACKER_LIMIT_QUEUES` они не фильтруются — проверяется только аргумент `queue` у `boards_get_all`, а в ответах этих инструментов могут встречаться запрещённые очереди.
...
- **`issue_add_checklist_items`**: Добавить один или несколько пунктов в чек-лист задачи (чек-лист создаётся, если его ещё нет)
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `items` (массив, обязательно): Пункты для добавления, по порядку. Каждый пункт принимает `text` (строка, обязательно), `checked` (булево, опционально), `assignee` (логин или uid, опционально) и `deadline` (объект, опционально: `{"date": "2026-08-20T00:00:00", "deadline_type": "date"}`)
  - Возвращает чек-лист задачи после добавления пунктов
  - Примечание: API Трекера принимает один пункт за запрос, поэтому пункты отправляются последовательно, в переданном порядке

- **`issue_update_checklist_item`**: Изменить один пункт чек-листа задачи, например отметить его выполненным
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `checklist_item_id` (строка, обязательно): ID пункта, как его возвращает `issue_get_checklist`
    - `text` (строка, опционально): Новый текст пункта; не передавайте, чтобы оставить текущий
    - `checked` (булево, опционально)
    - `assignee` (строка, опционально): Логин или uid
    - `deadline` (объект, опционально): `{"date": "2026-08-20T00:00:00", "deadline_type": "date"}` — `deadline_type` принимает `"date"` или `"quarter"`
  - Нужно передать хотя бы одно из `text`, `checked`, `assignee`, `deadline`; вызов, который ничего не меняет, отклоняется
  - Изменяются только переданные поля; остальные сохраняют текущие значения. Очистка не поддерживается: `null` в `assignee` или `deadline` оставит текущее значение, а не снимет его — чтобы убрать исполнителя или дедлайн, удалите пункт и добавьте заново
  - Если не передать `text`, текущий текст пункта сохранится — переотправлять его не нужно
  - Возвращает чек-лист задачи после изменения

- **`issue_delete_checklist_item`**: Удалить один пункт из чек-листа задачи
  - Параметры:
    - `issue_id` (строка, обязательно, формат: "QUEUE-123")
    - `checklist_item_id` (строка, обязательно): ID пункта, как его возвращает `issue_get_checklist`
  - Возвращает чек-лист задачи после удаления
