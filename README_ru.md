...
- **`project_update`**: Изменить любое из перечисленных полей существующего проекта. Принимает опциональные `comment` и `version` (для обнаружения конфликтов через оптимистичную блокировку). Переданный `links` заменяет весь набор связей сущности целиком. В API Трекера связи доступны только на запись (`links` не входит в допустимые значения `fields` и не возвращается ни get, ни update), поэтому прочитать текущий набор нельзя, а не переданные связи теряются
