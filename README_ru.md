...
- **`boards_get_all`**: Получить Agile-доски, доступные в организации
  - Параметры:
    - `queue` (строка, опционально): Вернуть только доски, которые собирают задачи этой очереди, например `"SOMEPROJECT"`
    - `fields` (массив строк, опционально): Поля, которые нужно вернуть. Выбор `["id", "name"]` при поиске доски уменьшает ответ примерно в 30 раз
    - `cursor` (целое число, опционально): Значение `next_cursor` из предыдущего вызова — это id последней вернувшейся доски. Для первой страницы оставьте пустым
    - `per_page` (целое число, опционально, по умолчанию 25): Количество досок на странице
  - Возвращает `{boards, next_cursor}`; каждая доска — с id, названием, версией, колонками, настройками и метаданными создания. Передавайте `next_cursor` обратно в `cursor`, пока он не вернётся `null`
  - **Как найти доску проекта**: у доски нет собственного поля с очередью — она собирает то, что подходит под её `autoFilterSettings`, поэтому `queue` сопоставляется именно с этим фильтром. Серверной фильтрации у Трекера здесь нет, поэтому инструмент фильтрует по ходу обхода списка, и `per_page` считает совпадения, а не прочитанные доски — очередь, единственная доска которой лежит за первой страницей, всё равно находится. Регистр не важен, доска с несколькими очередями находится по любой из них, а инвертированное условие («очередь не X») совпадением не считается. Доски, в фильтре которых очередь не указана вовсе, найти этим способом нельзя, и при заданном `queue` они не попадают в результат; сколько их — целиком зависит от того, как настроены доски в конкретной организации. Чтобы найти и их, прочитайте несколько задач очереди через `issues_find` и посмотрите на их поле `boards`: Трекер заполняет его по фильтрам самих досок, поэтому персональная доска, собирающая задачи одного исполнителя, там видна, даже если очередь в её фильтре не указана.
  - `queue` учитывает `TRACKER_LIMIT_QUEUES`: обращение к запрещённой очереди отклоняется
  - Используйте полученный `id` доски в инструментах `board_get`, `board_get_columns` и `board_get_sprints`
  - Навигация здесь курсорная, а не по номеру страницы, потому что именно это даёт `GET /v3/boards/_paginate`: доски приходят упорядоченными по id, а следующая страница начинается сразу за последним id предыдущей. Заданный `queue` по-прежнему означает чтение всех досок — совпадение может оказаться где угодно, — но это делается обходом страниц, а не запросом всей организации разом: запрос, размер которого растёт вместе с трекером, не покрывается никаким значением `TRACKER_API_TIMEOUT`, а страница одинакова при любом размере организации.

- **`board_get`**: Получить одну Agile-доску с её настройками
  - Параметры:
    - `board_id` (целое число, идентификатор доски из `boards_get_all`)
    - `fields` (массив строк, опционально): Поля, которые нужно вернуть
  - `autoFilterSettings` — это собственный фильтр доски: он говорит, какие задачи доска собирает, поэтому именно по нему видно, про какую очередь доска. `addFilterSettings` описывает, что попадает на доску, `removeFilterSettings` — что с неё уходит; условие задаётся либо фиксированным значением (очередь, тип задачи), либо макросом вроде `empty()` / `notEmpty()`
  - Также возвращает `estimateBy` (поле, по которому оценивают задачи), `useRanking` и производственный `calendar`, по которому считаются рабочие дни спринта
  - Несуществующий `board_id` возвращается как ошибка «доска не найдена»

- **`board_get_columns`**: Получить колонки Agile-доски вместе со статусами задач
  - Параметры: `board_id` (целое число, идентификатор доски из `boards_get_all`)
  - Возвращает каждую колонку с `id`, `name` и списком статусов задач `statuses`, которые в неё попадают — по нему видно, в какой статус нужно перевести задачу, чтобы она оказалась в нужной колонке
  - Богаче, чем колонки внутри `boards_get_all` / `board_get`: там статусов нет

- **`board_get_sprints`**: Получить все спринты конкретной Agile-доски
  - Параметры:
    - `board_id` (целое число, идентификатор доски из `boards_get_all`)
    - `fields` (массив строк, опционально): Поля, которые нужно вернуть
  - У доски, которая не является scrum-доской, спринтов нет, и API отклоняет вызов с сообщением «У доски этого типа не может быть спринтов.»; несуществующий `board_id` возвращается как ошибка «доска не найдена»
  - Возвращает список спринтов с id, названием, статусом, признаком архивности, плановыми датами (`startDate`, `endDate`) и фактическими датами (`startDateTime`, `endDateTime`)
  - Статус спринта — одно из значений `draft`, `in_progress`, `released` или `archived`; текущий спринт — тот, у которого статус `in_progress`
  - Используйте полученный `id` спринта, чтобы поместить задачу в спринт с помощью `issue_create` или `issue_update`

**Инструменты досок и `TRACKER_LIMIT_QUEUES`.** Доска относится ко всей организации и не имеет собственной очереди, поэтому — в отличие от очередей, задач и шаблонов — доски по allow-list не фильтруются: такая фильтрация лишила бы эти инструменты смысла. Значит, запрещённая очередь всё равно может быть названа в том, что они возвращают: `boards_get_all` без `queue` перечисляет все доски организации вместе с `autoFilterSettings`, где лежат ключи и отображаемые имена очередей, а `board_get` / `board_get_columns` / `board_get_sprints` отвечают по любому идентификатору доски. Единственная проверка, которая здесь работает, — на аргументе `queue` у `boards_get_all`: запрещённая очередь отклоняется. Если allow-list у вас единственная граница (например, общий MCP-шлюз), рассчитывайте на то, что инструменты досок раскроют ключи и названия очередей за её пределами.

У задачи, полученной через `issue_get` / `issues_find`, есть поле `boards` со списком досок, на которых она отображается (`{"id": 2, "name": "..."}`) — этот `id` можно сразу передавать в инструменты выше. Трекер заполняет `boards` по фильтрам самих досок, поэтому поле только для чтения: задачу нельзя назначить на доску напрямую, она просто попадает под её фильтр.
...
- **`TRACKER_LIMIT_QUEUES`** — список разрешённых ключей очередей. Очереди вне
  ...
  настройках доски, см. *Инструменты досок и `TRACKER_LIMIT_QUEUES`* выше.
