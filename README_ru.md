...
- **`boards_get_all`**: Получить Agile-доски, доступные в организации
  ...
  - У `GET /v3/boards` нет собственной постраничной навигации, он отдаёт сразу все доски организации, поэтому инструмент разбивает результат на страницы сам: в организации с сотнями досок один вызов иначе вернул бы четверть мегабайта JSON. Увеличивайте `page`, пока результат не станет пустым.
  - Доски относятся ко всей организации, поэтому не фильтруются настройкой `TRACKER_LIMIT_QUEUES`
