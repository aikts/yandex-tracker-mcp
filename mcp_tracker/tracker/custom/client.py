...
class TrackerClient(QueuesProtocol, IssueProtocol, GlobalDataProtocol, UsersProtocol):
    ...
    async def queues_list(
        self, per_page: int = 100, page: int = 1, *, token: str | None = None
    ) -> list[Queue]:
        params = {
            "perPage": per_page,
            "page": page,
        ...
        async with self._session.get(
            "v3/queues", headers=self._build_headers(token), params=params
        ) as response:
            response.raise_for_status()
            return QueueList.model_validate_json(await response.read()).root

    async def queues_get_local_fields(
        self, queue_id: str, *, token: str | None = None
    ) -> list[LocalField]:
        async with self._session.get(
            f"v3/queues/{queue_id}/localFields", headers=self._build_headers(token)
        ) as response:
            response.raise_for_status()
    ...
    async def queues_get_tags(
        ...
        async with self._session.get(
            f"v3/queues/{queue_id}/tags", headers=self._build_headers(token)
        ) as response:
            response.raise_for_status()
    ...
    async def queues_get_versions(
        ...
        async with self._session.get(
            f"v3/queues/{queue_id}/versions", headers=self._build_headers(token)
        ) as response:
            response.raise_for_status()
            return VersionList.model_validate_json(await response.read()).root

    async def get_global_fields(self, *, token: str | None = None) -> list[GlobalField]:
        async with self._session.get(
            "v3/fields", headers=self._build_headers(token)
        ) as response:
            response.raise_for_status()
    ...
    async def get_statuses(self, *, token: str | None = None) -> list[Status]:
        async with self._session.get(
            "v3/statuses", headers=self._build_headers(token)
        ) as response:
            response.raise_for_status()
    ...
    async def get_issue_types(self, *, token: str | None = None) -> list[IssueType]:
        async with self._session.get(
            "v3/issuetypes", headers=self._build_headers(token)
        ) as response:
            response.raise_for_status()
            return IssueTypeList.model_validate_json(await response.read()).root

    async def issue_get(
        self, issue_id: str, *, token: str | None = None
    ) -> Issue | None:
        async with self._session.get(
            f"v3/issues/{issue_id}", headers=self._build_headers(token)
        ) as response:
    ...
    async def issues_get_links(
        ...
        async with self._session.get(
            f"v3/issues/{issue_id}/links", headers=self._build_headers(token)
        ) as response:
            if response.status == 404:
                return None
            response.raise_for_status()
    ...
    async def issue_get_comments(
        ...
        async with self._session.get(
            f"v3/issues/{issue_id}/comments", headers=self._build_headers(token)
        ) as response:
            if response.status == 404:
                return None
            response.raise_for_status()
    ...
    async def issues_find(
        self, query: str, *, per_page: int = 15, page: int = 1, token: str | None = None
    ) -> list[Issue]:
        params = {
            "perPage": per_page,
            "page": page,
    ...
    async def issue_get_worklogs(
        ...
        async with self._session.get(
            f"v3/issues/{issue_id}/worklog", headers=self._build_headers(token)
        ) as response:
            if response.status == 404:
                return None
            response.raise_for_status()
    ...
    async def issue_get_attachments(
        ...
        async with self._session.get(
            f"v3/issues/{issue_id}/attachments", headers=self._build_headers(token)
        ) as response:
            if response.status == 404:
                return None
            response.raise_for_status()
    ...
    async def users_list(
        ...
        async with self._session.get(
            "v3/users", headers=self._build_headers(token), params=params
        ) as response:
            response.raise_for_status()
            return UserList.model_validate_json(await response.read()).root

    async def user_get(self, user_id: str, *, token: str | None = None) -> User | None:
        async with self._session.get(
            f"v3/users/{user_id}", headers=self._build_headers(token)
        ) as response:
            if response.status == 404:
                return None
            response.raise_for_status()
    ...
    async def issues_count(self, query: str, *, token: str | None = None) -> int:
        body: dict[str, Any] = {
            "query": query,
        }

        async with self._session.post(
            "v3/issues/_count", headers=self._build_headers(token), json=body
        ) as response:
            response.raise_for_status()
