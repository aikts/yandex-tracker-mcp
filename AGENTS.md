...
- **A board has no queue field.** What lands on it is whatever `autoFilterSettings` matches, so "boards of queue X" is answered by reading the queue out of that filter (`board_queue_keys()` in `mcp/tools/board.py`), and `?queue=` on the endpoint is ignored like the paging parameters. About a third of a real organization's boards name no queue in their filter and cannot be found this way at all - a queue-scoped listing has to say so rather than imply the queue has no boards. The second route covers them: an issue's `boards` is filled in by Tracker from the boards' own filters, so reading a few issues of the queue surfaces the personal and assignee-filtered boards that `queue` cannot match (verified: `DEVOPSOBR` matches 2 boards by filter and 6 through its issues). The server `instructions` tell agents to try both.
